...
impl<F: Field + PrimeFieldBits> RangeConstrained<F, AssignedCell<F, F>> {
...
pub struct LookupRangeCheckConfig<F: Field + PrimeFieldBits, const K: usize> {
    q_lookup: Selector,
    q_running: Selector,
    q_bitshift: Selector,
...
impl<F: Field + PrimeFieldBits, const K: usize> LookupRangeCheckConfig<F, K> {
...
mod tests {
    ...
    fn lookup_range_check() {
        #[derive(Clone, Copy)]
        struct MyCircuit<F: Field + PrimeFieldBits> {
            num_words: usize,
            _marker: PhantomData<F>,
        }

        impl<F: Field + PrimeFieldBits> Circuit<F> for MyCircuit<F> {
            ...
            fn synthesize(
                ...
                fn expected_zs<F: Field + PrimeFieldBits, const K: usize>(
                    element: F,
                    num_words: usize,
                ) -> Vec<F> {
                    let chunks = {
